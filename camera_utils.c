...
/*   Updated: 2026/03/01 18:25:21 by asadik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	reset_zoom(t_state *state)
{
	...
	{
		if (world_w > world_h)
			state->camera.zoom.current = (state->window_size.x * 0.5) / world_w;
		else
			state->camera.zoom.current = (state->window_size.y * 0.5) / world_h;
	}
